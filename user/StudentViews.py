...
def student_view_attendance(request):
    ...
    for subject in subjects:
        ...
        tot_lec = Attendance.objects.filter(subject_id=subject).count()
        print(tot_lec)
        total_lec.append(tot_lec)
        pres_lec = AttendanceReport.objects.filter(attendance_id__in=attendance,
                                                   student_id=stud_obj).count()
        present_lec.append(pres_lec)
        per=pres_lec/tot_lec*100
        perc.append(per)
        

    
    index = range(len(total_lec))
    subjects = list(zip(subjects, total_lec, present_lec,perc))
    context = {
        "subjects": subjects,
